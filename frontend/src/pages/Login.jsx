    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (response?.data) {
                ...
                localStorage.setItem("name", response.data.name || "Enterprise User");
                localStorage.setItem("role", response.data.role || "Procurement Admin");
            }
            navigate("/dashboard");
        } catch (error) {
            console.log(error);
            // Fallback for demonstration / backend off-line mode
            localStorage.setItem("userId", "1");
            localStorage.setItem("name", "Procurement Admin");
